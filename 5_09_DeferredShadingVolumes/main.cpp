//
// deferred shading volumes，延迟着色法，使用光体积(Light Volumes)
//
...
int main()
{
    ...
    //延迟渲染能够渲染大量的光源而不消耗大量的性能。然而，延迟渲染它本身并不能支持非常大量的光源，因为我们仍然必须要对场景中每一个光源计算每一个片段的光照分量。真正让大量光源成为可能的是我们能够对延迟渲染管线引用的一个非常棒的优化：光体积(Light Volumes)
    //通常情况下，当我们渲染一个复杂光照场景下的片段着色器时，我们会计算场景中每一个光源的贡献，不管它们离这个片段有多远。但是很大一部分的光源根本就不会到达这个片段，所以为什么我们还要浪费这么多光照运算呢？
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (unsigned int i = 0; i < lightPositions.size(); i++)
        {
            ...
            //计算光体积(Light Volumes/Sphere)的半径
