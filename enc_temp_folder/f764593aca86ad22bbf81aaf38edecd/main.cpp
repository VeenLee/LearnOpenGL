...
int main()
{
    ...
    pbrShader.setVec3("albedo", 200.0f / 255.0f, 200.0f / 255.0f, 200.0f / 255.0f);
    ...
    glm::mat4 captureViews[] =
    {
        ...
        glm::lookAt(glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, -1.0f,  0.0f), glm::vec3(0.0f,  0.0f, -1.0f)),  //下，上下翻转
    ...
    for (unsigned int i = 0; i < 6; ++i)
    {
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB16F, prefilterMapWidth, prefilterMapHeight, 0, GL_RGB, GL_FLOAT, nullptr);
    }
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
    ...
    for (unsigned int mip = 0; mip < maxMipLevels; ++mip)
    {
        ...
        unsigned int mipWidth = static_cast<unsigned int>(prefilterMapWidth * std::pow(0.5, mip));
        unsigned int mipHeight = static_cast<unsigned int>(prefilterMapHeight * std::pow(0.5, mip));
        glBindRenderbuffer(GL_RENDERBUFFER, captureRBO);
